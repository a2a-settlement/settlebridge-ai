 ...
 * Detects AlphaSignal ensemble forecast payloads, structured "analyst report" payloads,
 * and falls back to a generic sectioned document view.
 */
...
/** Bounty acceptance criteria often require `forecast_bands` + `last_close` instead of `predictions` + `last_actual_close`. */
...
/** Map bounty-schema payloads to the shape `EnsembleForecastView` expects. */
function normalizeEnsemblePayload(obj: Record<string, unknown>): Record<string, unknown> {
  const out = { ...obj };
  if (!Array.isArray(out.predictions) && Array.isArray(out.forecast_bands)) {
    out.predictions = out.forecast_bands;
  }
  if (out.last_actual_close == null && typeof out.last_close === "number") {
    out.last_actual_close = out.last_close;
  }
  return out;
}
...
interface ForecastIteration {
  iteration: number;
  iteration_delta: string;
  deliverable?: Record<string, unknown>;
}

function getForecastChartUrl(
...
function EnsembleForecastView({ obj }: { obj: Record<string, unknown> }) {
  ...
  const horizon = obj.forecast_horizon_days as number;
  const lastClose =
    typeof obj.last_actual_close === "number" ? obj.last_actual_close : null;
  const predictions = obj.predictions as Prediction[];
  const showDay = predictions.some((p) => p.day != null);
  const modelWeights = obj.model_weights as Record<string, number>;
  const iterations = Array.isArray(obj.iterations)
    ? (obj.iterations as ForecastIteration[])
    : [];
  const verif = asRecord(obj.verification_artifacts);
  const chartUrl = getForecastChartUrl(obj, ticker, predictions);

  const lastPred = predictions[predictions.length - 1];
  const directionUp =
    lastClose != null ? lastPred.mean >= lastClose : true;

  const maxWeight = Math.max(...Object.values(modelWeights).map(Number));
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        {lastClose != null ? (
          <div className="bg-navy-50 rounded-xl p-3 text-center">
            <div className="text-xs text-gray-500 mb-1">Last Close</div>
          ...
          <div className="bg-navy-50 rounded-xl p-3 text-center">
            ...
            <div
              className={`text-2xl font-bold ${
                directionUp ? "text-emerald-600" : "text-rose-600"
              }`}
            >
              {directionUp ? "▲" : "▼"}
            </div>
      ...
      <div>
        <h4 className="text-sm font-semibold text-navy-900 mb-2">
          ...
          {lastClose != null && (
            ...
            >
              {directionUp ? "▲" : "▼"} Target ${lastPred.mean.toFixed(2)}
            </span>
          )}
        </h4>
          ...
          <table className="min-w-full text-xs">
            <thead>
              <tr className="bg-gray-50 text-left">
                {showDay && (
                  <th className="px-3 py-2 text-gray-600 font-semibold">Day</th>
                )}
            ...
            <tbody>
              {predictions.map((p, i) => {
                const up = lastClose != null ? p.mean >= lastClose : p.mean >= predictions[0].mean;
                return (
                  <tr
                    key={i}
                  ...
                  >
                    {showDay && (
                      <td className="px-3 py-1.5 font-mono text-gray-500 text-center">
                        {p.day ?? "—"}
                      </td>
                    )}
                    <td className="px-3 py-1.5 font-mono text-gray-600">{p.date}</td>
                    <td
                      className={`px-3 py-1.5 text-right font-semibold font-mono ${
              ...
              .map(([model, weight]) => {
                ...
                return (
                  <div key={model} className="flex items-center gap-3">
                    ...
                    <div className="w-12 text-right text-xs font-mono text-gray-700 shrink-0">
                      {pct}%
                    </div>
                  </div>
                );
      ...
      {iterations.length > 0 && (
        <div className="border border-gray-200 rounded-xl overflow-hidden">
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-2.5 bg-gray-50 text-left hover:bg-gray-100 transition"
          >
            <span className="text-xs font-semibold text-gray-700">
              Training Iterations ({iterations.length})
            </span>
            {showIterations ? (
          ...
          {showIterations && (
            <div className="divide-y divide-gray-100">
              {iterations.map((iter) => (
                <div key={iter.iteration} className="px-4 py-2.5 text-xs flex gap-2.5">
                  ...
                  <span className="text-gray-600 leading-relaxed">{iter.iteration_delta}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="border border-gray-200 rounded-xl overflow-hidden">
          ...
          {showVerification && (
            <div className="px-4 py-3 space-y-2 text-xs">
              {typeof verif.endpoint_url === "string" && verif.endpoint_url && (
                <div>
                  <span className="text-gray-500">Endpoint:</span>{" "}
                  <a
                    href={verif.endpoint_url}
                    target="_blank"
                    rel="noreferrer"
                    className="text-navy-600 hover:underline font-mono break-all"
                  >
                    {verif.endpoint_url}
                  </a>
                </div>
              )}
              {verif.content_hash != null && (
                <div>
                  <span className="text-gray-500">Content hash (SHA-256):</span>{" "}
                  <span className="font-mono text-gray-700 break-all">
                    {String(verif.content_hash)}
                  </span>
                </div>
              )}
              {Object.entries(verif)
                .filter(([k]) => !["endpoint_url", "content_hash", "api_call_raw"].includes(k))
                .map(([k, v]) => (
                  <div key={k}>
                    <span className="text-gray-500 capitalize">{k.replace(/_/g, " ")}:</span>{" "}
                    <span className="text-gray-700">{fmtValue(v)}</span>
                  </div>
                ))}
            </div>
          )}
...
export default function JsonDeliverablePreview({ content }: { content: string }) {
  ...
  if (isEnsembleForecast(obj)) {
    return <EnsembleForecastView obj={normalizeEnsemblePayload(obj)} />;
  }
  ...
  return <>{GenericDocumentSections(obj)}</>;
}
