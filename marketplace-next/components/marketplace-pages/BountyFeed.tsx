...
import type { Bounty, BountyListResponse, TrainingCardData } from "@/lib/types";
import BountyCard from "@/components/BountyCard";
import TrainingRunCard from "@/components/TrainingRunCard";
import CategoryFilter from "@/components/CategoryFilter";

type ViewMode = "open" | "in_progress" | "completed";
type ResultType = "all" | "bounties" | "training";

const IN_PROGRESS_STATUSES = ["claimed", "submitted", "in_review", "disputed"] as const;

function subtitleFor(viewMode: ViewMode, total: number): string {
  if (viewMode === "open") return `${total} bounties available`;
  if (viewMode === "in_progress") return `${total} in progress`;
...
export default function BountyFeed() {
  ...
  // Fetch in-progress bounties (claimed / submitted / in_review / disputed)
  useEffect(() => {
    if (viewMode !== "in_progress") return;
    setLoading(true);
    const params = new URLSearchParams();
    IN_PROGRESS_STATUSES.forEach((s) => params.append("status", s));
    params.set("page", String(page));
    params.set("page_size", "12");
  ...
  const totalPages =
    viewMode === "open" || viewMode === "in_progress"
      ? Math.ceil(total / 12)
      : 1;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex gap-1 mb-5 bg-gray-100 rounded-lg p-1 w-fit">
        <button onClick={() => setViewMode("open")} className={tabClass("open")}>
          Open Bounties
        </button>
        <button
          onClick={() => setViewMode("in_progress")}
          className={tabClass("in_progress")}
        >
          In Progress
        </button>
        <button
          ...
          {loading ? (
            ...
            bounties.length === 0 ? (
              <div className="text-center py-20 text-gray-400">
                ...
                <p className="text-sm">
                  {viewMode === "in_progress"
                    ? "Claimed and in-review bounties will show up here"
