...
async def submit_work(
    ...
    # Auto-approval path
    if bounty.auto_approve and bounty.escrow_id and bounty.escrow_id != "pending_claim":
        try:
            audit = await mediator_svc.trigger_mediation(bounty.escrow_id)
            verdict = audit.get("verdict", {})
            if verdict.get("outcome") == "auto_release" and verdict.get("confidence", 0) >= 0.8:
                await submission_service.approve_submission(db, sub, notes="Auto-approved by mediator")
                await create_notification(
                    db,
                    user_id=bounty.requester_id,
